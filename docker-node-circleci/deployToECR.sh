...
check_git_repo_requirements() {
  if ! git diff-index --quiet HEAD --; then
    echo "Your git repo has untracked changes, please fix this before commiting code by accident."
    git status
    exit 1
  fi
}
...
push_images_to_docker() {
  ...
  while read -r line; do
    ...
    echo "Pushing local build of service $SERVICE_NAME to ECR"
...
strip_build_options() {
  node /usr/local/removeBuildFromYaml/removeBuildFromYaml.js $TEMPFILE
}
